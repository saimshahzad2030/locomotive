...
const Navbar = () => {
  ...
  return (
    <div className="relative">
      {/* Navbar */}
      <div className="w-full h-[100px] dark:bg-white bg-slate-900 dark:text-white text-slate-900 flex flex-row items-center justify-between px-4">
        <Link href={"/about"}>
        ...
        <div className={"flex flex-row items-center"}>
          {/* <button
            ...
            className="p-2   text-white dark:text-slate-900 rounded-full focus:outline-none flex flex-row items-center"
          >
            {"Theme: "}
            {theme === "dark" ? (
              <SUN className=" ml-1 w-6 h-6   text-slate-900" />
            ) : (
              <MOON className="ml-1 w-6 h-6 text-white" />
            )}
          </button> */}
          <HAMBURGER
            className="w-12 h-12 dark:text-slate-900 text-white cursor-pointer"
      ...
      <div
        className={`z-50 fixed top-0 right-0 h-screen w-screen dark:bg-white bg-slate-900 dark:text-slate-900 text-white transform transition-transform duration-1000 ${
      ...
      >
        <div className="flex flex-col h-full justify-center items-center space-y-8">
          ...
          >
            <Link href="https://facebook.com">
              <FACEBOOK
                className={
                  "w-8 h-8 text-white dark:text-slate-900    transform hover:scale-125 transition-transform duration-200 "
                }
              />
            </Link>
        ...
        {/* Close Button */}
        <CROSS
          className="absolute top-4 right-4 dark:text-slate-900 text-white w-8 h-8 cursor-pointer"
